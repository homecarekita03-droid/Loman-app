...
export default function SellerDashboard() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      // Skip vibrate saat pertama kali load
      if (isFirstLoad.current) {
        isFirstLoad.current = false;
        if (orderList.length > 0) lastOrderId.current = orderList[0].id;
        return;
      }

      // Deteksi pesanan baru
      if (orderList.length > 0 && orderList[0].id !== lastOrderId.current) {
        ...
        lastOrderId.current = orderList[0].id;
      }
    }, (error) => {
  ...
  const active = orders.filter((o) =>
    ["confirmed", "processing", "delivering"].includes(o.status)
  );
  const nextStatus = { confirmed: "processing", processing: "delivering", delivering: "done" };
  const nextLabel = { confirmed: "Mulai Proses 🔄", processing: "Kirim 🛵", delivering: "Selesai ✅" };
  const statusLabel = { confirmed: "✅ Diterima", processing: "🔄 Diproses", delivering: "🛵 Diantar" };

  async function toggleStore() {
    if (!store) return;
    const newState = !store.isOpen;
    await updateDoc(doc(db, "toko", store.id), { isOpen: newState });
    setStore((prev) => ({ ...prev, isOpen: newState }));
  }

  async function updateStatus(id, newStatus) {
    await updateDoc(doc(db, "pesanan", id), { status: newStatus });
    // onSnapshot akan update otomatis, tapi kita juga update lokal untuk responsif
  }
  ...
  if (chatOrder) {
    return (
      <ChatRoom
        pesananId={chatOrder.pesananId || chatOrder.id}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f5f5", paddingBottom: "80px" }}>
      ...
      {showChatList && (
        <ChatList
          onOpenChat={(c) => { setShowChatList(false); setChatOrder(c); }}
      ...
      <div style={{ background: "linear-gradient(135deg,#f59e0b,#ea580c)", padding: "20px 20px 28px", borderRadius: "0 0 24px 24px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            <button onClick={() => setShowNotif(true)} style={{ width: "40px", height: "40px", borderRadius: "50%", background: "rgba(255,255,255,0.2)", border: "none", cursor: "pointer", fontSize: "18px", position: "relative" }}>
              🔔
              {pending.length > 0 && (
                <div style={{ position: "absolute", top: "-2px", right: "-2px", width: "18px", height: "18px", borderRadius: "50%", background: "#ef4444", color: "white", fontSize: "10px", fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {pending.length}
                </div>
              )}
            </button>
        ...
        <div style={{ display: "flex", gap: "10px", marginTop: "16px" }}>
          <div style={{ flex: 1, background: "rgba(255,255,255,0.15)", borderRadius: "14px", padding: "14px" }}>
            <div style={{ fontSize: "24px", fontWeight: 900, color: "white" }}>{todayOrders.length}</div>
            <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.7)" }}>📦 Pesanan Hari Ini</div>
          </div>
          <div style={{ flex: 1, background: "rgba(255,255,255,0.15)", borderRadius: "14px", padding: "14px" }}>
            <div style={{ fontSize: "20px", fontWeight: 900, color: "white" }}>
              Rp {todayRevenue >= 1000 ? Math.round(todayRevenue / 1000) + "K" : todayRevenue}
            </div>
            <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.7)" }}>💰 Pendapatan Hari Ini</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: "16px 20px 0", display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "8px" }}>
        {[
          ...
          <button key={a.label} onClick={() => router.push(a.path)} style={{ padding: "12px 4px", borderRadius: "14px", background: "white", border: "none", cursor: "pointer", textAlign: "center", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
            ...
            <div key={o.id} style={{ background: "white", borderRadius: "16px", padding: "16px", marginBottom: "10px", boxShadow: "0 2px 8px rgba(0,0,0,0.04)", borderLeft: "4px solid #f59e0b" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  ...
                  <div>
                    <h4 style={{ fontSize: "14px", fontWeight: 700 }}>{o.pembeliNama}</h4>
                    <p style={{ fontSize: "11px", color: "#9ca3af" }}>{o.pembeliAlamat || "-"}</p>
                  </div>
                </div>
              ...
              <div style={{ background: "#f9fafb", borderRadius: "10px", padding: "10px", marginBottom: "10px" }}>
                {o.items?.map((it, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", padding: "2px 0" }}>
                    ...
                    <span style={{ color: "#6b7280" }}>Rp {(it.harga * it.qty).toLocaleString("id")}</span>
                  </div>
                ))}
              </div>
          ...
          {active.map((o) => (
            <div key={o.id} style={{ background: "white", borderRadius: "16px", padding: "14px", marginBottom: "10px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                ...
                <span style={{ fontSize: "11px", padding: "4px 10px", borderRadius: "20px", fontWeight: 600, background: o.status === "confirmed" ? "#dbeafe" : o.status === "processing" ? "#fef3c7" : "#e0e7ff", color: o.status === "confirmed" ? "#2563eb" : o.status === "processing" ? "#d97706" : "#7c3aed" }}>
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                ...
                {nextStatus[o.status] && (
                  <button onClick={() => updateStatus(o.id, nextStatus[o.status])} style={{ flex: 1, padding: "10px", borderRadius: "12px", background: "linear-gradient(135deg,#f59e0b,#ea580c)", color: "white", border: "none", fontWeight: 700, fontSize: "13px", cursor: "pointer" }}>
                    {nextLabel[o.status]}
                  </button>
                )}
              </div>
